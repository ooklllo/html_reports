...
try {
    ...
    foreach ($file in $sortedFiles) {
        ...
        $cardsHtml += @"
      <a href="$($file.Name)" class="report-card$latestClass">
        <div class="card-left">
          <div class="card-meta">
            $badgeHtml
            <span class="card-date">$dateStr</span>
            <span class="card-filename">$($file.Name) (${fileSizeKb}KB)</span>
          </div>
          <div class="card-title">$title</div>
        </div>
        <div class="card-right">
          <span class="btn-open">보고서 열기 →</span>
        </div>
      </a>
...
"@
    ...
    if ($uncommittedFiles.Count -gt 0) {
        ...
        $commitMsg = "Add/Update report: $filesStr ($(Get-Date -Format 'yyyy-MM-dd HH:mm'))"
    } else {
        $commitMsg = "Refresh report index ($(Get-Date -Format 'yyyy-MM-dd HH:mm'))"
    }
    ...
    if ($newReportUrls.Count -gt 0) {
        ...
        Write-Host "   - 새 보고서 링크 ($($newReportUrls.Count)개)가 클립보드에 복사되었습니다! (Ctrl+V 로 공유 가능)" -ForegroundColor Green
